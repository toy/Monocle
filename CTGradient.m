...
- (CTGradient *)removeColorStopAtPosition:(float)position {
  ...
  if (isnan(removedElement.position))
    [NSException raise:NSRangeException
                format:@"-[%@ removeColorStopAtPosition:]: no such colorStop at position (%f)", [self class], position];
...
- (CTGradient *)removeColorStopAtIndex:(unsigned)index {
  ...
  if (isnan(removedElement.position))
    [NSException raise:NSRangeException
                format:@"-[%@ removeColorStopAtIndex:]: index (%i) beyond bounds", [self class], index];
...
- (void)radialFillRect:(NSRect)rect {
  ...
  CGShadingRef myCGShading =
    CGShadingCreateRadial(colorspace, startPoint, startRadius, endPoint, endRadius, gradientFunction, true, true);
...
- (void)setBlendingMode:(CTGradientBlendingMode)mode;
{
  ...
  CGFunctionCallbacks evaluationCallbackInfo = {0,
    evaluationFunction,
    NULL};  // Version, evaluator function, cleanup function

  static const float input_value_range[2] = {0, 1};  // range  for the evaluator input
  static const float output_value_ranges[8] =
    {0, 1, 0, 1, 0, 1, 0, 1};  // ranges for the evaluator output (4 returned values)

  gradientFunction = CGFunctionCreate(&elementList,  // the two transition colors
    1,
    input_value_range,  // number of inputs (just fraction of progression)
    4,
    output_value_ranges,       // number of outputs (4 - RGBa)
...
- (void)addElement:(CTGradientElement *)newElement {
  ...
  } else  // inserting somewhere inside list
  {
    ...
    while (curElement->nextElement != nil &&
      !((curElement->position <= newElement->position) && (newElement->position < curElement->nextElement->position))) {
      curElement = curElement->nextElement;
    }
...
void transformRGB_HSV(float *components)  // H,S,B -> R,G,B
{
  ...
  float MAX = R > G ? (R > B ? R : B) : (G > B ? G : B), MIN = R < G ? (R < B ? R : B) : (G < B ? G : B);
...
void transformHSV_RGB(float *components)  // H,S,B -> R,G,B
{
  ...
  float H = fmodf(components[0], 359),  // map to [0,360)
    S = components[1], V = components[2];

  int Hi = (int)floorf(H / 60.) % 6;
  float f = H / 60 - Hi, p = V * (1 - S), q = V * (1 - f * S), t = V * (1 - (1 - f) * S);
