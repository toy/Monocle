...
+ (void)initialize {
  [self setKeys:[NSArray arrayWithObject:@"discoveredEngines"]
    triggerChangeNotificationsForDependentKey:@"anyEnginesDetected"];
}
...
- (void)webView:(WebView *)sender didStartProvisionalLoadForFrame:(WebFrame *)frame {
  ...
  if (frame == [sender mainFrame]) {
    if (currentURL != nil &&
      [[[currentURL host] lowercaseString]
        isNotEqualTo:[[[[[frame provisionalDataSource] request] URL] host] lowercaseString]]) {
...
- (BOOL)tryToDiscoverSearchEngineFromSubmittedForm:(NSDictionary *)actionInformation
                                           request:(NSURLRequest *)request
                                             frame:(WebFrame *)frame {
  ...
  if (forms == 0) { /*NSLog(@"no forms!? crapping out.");*/
    return NO;
  }
  unsigned long i = 0;
  ...
  if ([originalURL query] && [[originalURL query] isNotEqualTo:@""]) {
    origwoGET = [NSURL
      URLWithString:[[originalAbs componentsSeparatedByString:[NSString stringWithFormat:@"?%@", [originalURL query]]]
                      objectAtIndex:0]];
  ...
  for (i = 0; i < forms; i++) {
    ...
    // NSLog(@"text encoding name: %@; translated into int: %d", [resp textEncodingName], [NSString
    // stringEncodingForIANA:[resp textEncodingName]]);
    ...
    if (!elements) { /*NSLog(@"no inputs!");*/
      ...
      if (numEl > 0) {
        ...
        for (j = 0; j < numEl; j++) {
          ...
          if (([type isNotEqualTo:@"file"]) && ([type isNotEqualTo:@"radio"]) && ([type isNotEqualTo:@"check"]) &&
            ([type isNotEqualTo:@"checkbox"]) && ([type isNotEqualTo:@"password"]) && ([type isNotEqualTo:@"submit"]) &&
            ([type isNotEqualTo:@"image"]) && ([type isNotEqualTo:@"reset"]) && ([type isNotEqualTo:@"button"]) &&
    ...
    if (!elements) { /*NSLog(@"no textareas!");*/
    } else {
      unsigned long numEl = [elements length];
    ...
    if ([texts count] == 1) {
      ...
      //			NSString *encWord = [[[word stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]
      // componentsSeparatedByString:@"%20"] componentsJoinedByString:@"+"];
      ...
      if ([newqu length] > 2) newqu = [newqu substringToIndex:[newqu length] - 2];
      //			NSLog(@"after!");

      NSString *correctURL =
        [[[originalURL absoluteString] componentsSeparatedByString:[NSString stringWithFormat:@"?%@", qu]]
          componentsJoinedByString:[NSString stringWithFormat:@"?%@", newqu]];
      ...
      if ([[request HTTPMethod] isEqualToString:@"GET"]) {
        [self addDiscoveredEngine:[NSString stringWithFormat:@"%@ [searched for %@]",
                                            [self figureOutEngineName:[searchEngine stringValue]],
                                            word]
        ...
        if ([[request HTTPBody] isNotEqualTo:[NSData data]]) {
          ...
          NSSet *encodings = [NSSet setWithObjects:EmbEnc(NSUTF8StringEncoding),
                                    EmbEnc(NSASCIIStringEncoding),
                                    EmbEnc(NSISOLatin1StringEncoding),
                                    EmbEnc(NSShiftJISStringEncoding),
                                    EmbEnc(NSWindowsCP1252StringEncoding),
                                    EmbEnc(NSISO2022JPStringEncoding),
                                    EmbEnc(NSJapaneseEUCStringEncoding),
                                    nil];
          NSString *body;
...
- (NSString *)ianaEncodingForFrame:(WebFrame *)frame {
  ...
  if (nil != [frame DOMDocument]) {
    ...
    for (i = 0; i < ml; i++) {
      ...
      // NSLog(@"meta-node: %@, class: %@, node name: %@, node value: %@, node type: %d", n, NSStringFromClass([n
      // class]), [n nodeName], [n nodeValue], [n nodeType]);
      DOMHTMLMetaElement *m = (DOMHTMLMetaElement *)n;
      // NSLog(@"meta: %@ tagname: %@ http-equiv: %@, name: %@, content: %@", m, [m tagName], [m httpEquiv], [m name],
      // [m content]);
      if ([[[m httpEquiv] lowercaseString] isEqualToString:@"content-type"]) {
        ...
        NSString *contentType = [[contentTypeComponents lastObject]
    ...
    if (metaCharSet != nil) return metaCharSet;
  }
  if (nil != [ds mainResource]) {
    // NSLog(@"main resource encoding name: %@", [[ds mainResource] textEncodingName]);
    if (nil != [[ds mainResource] textEncodingName]) return [[ds mainResource] textEncodingName];
  }
  if (nil != [ds response]) {
    // NSLog(@"response encoding name: %@", [[ds response] textEncodingName]);
    if (nil != [[ds response] textEncodingName]) return [[ds response] textEncodingName];
  ...
  if ([[req HTTPMethod] isEqualToString:@"GET"]) {
    if ([[req URL] isOkayForDiscovery]) {
      if ([[[[req URL] description] componentsSeparatedByString:MagicWord] count] > 1) {
        // NSLog(@"Search URL: %@", [[[[req URL] description] componentsSeparatedByString:MagicWord]
        // componentsJoinedByString:@"%@"]);

        [self addDiscoveredEngine:[self figureOutEngineName:[searchEngine stringValue]]
                             icon:[[[pageIcon image] copy] autorelease]
                              url:[[[[req URL] description] componentsSeparatedByString:MagicWord]
                                    componentsJoinedByString:@"%@"]
                         postData:nil
    ...
    if ([[req HTTPBody] isNotEqualTo:[NSData data]]) {
      ...
      while (enc = UnbEnc([encEnumerator nextObject])) {
        ...
        if ([[body componentsSeparatedByString:MagicWord] count] > 1) {
          NSMutableDictionary *postData = [[NSDictionary dictionary] mutableCopy];
          NSArray *kvp = [[[body componentsSeparatedByString:MagicWord] componentsJoinedByString:@"%@"]
...
- (void)awakeFromNib {
  [discoveredEnginesArrayController addObserver:self
...
- (void)addDiscoveredEngine:(NSString *)title
                       icon:(NSImage *)icon
                        url:(NSString *)url
                   postData:(NSDictionary *)postData
               ianaEncoding:(NSString *)enc {
  BOOL isPOST = (postData ? YES : NO);
  //	NSLog(@"add discovered engine: %@, icon: %@, url: %@, post data: %@", title, icon, url, (postData ? postData :
  //@"nil"));
  ...
  if (![takenURLs containsObject:url]) {
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:title,
                                       ...
                                       enc,
                                       @"encoding",
                                       [NSNumber numberWithInt:(isPOST ? 1 : 0)],
                                       @"isPOST",
                                       [NSNumber numberWithInt:++counter],
                                       @"uid",
    ...
    NSLog(@"added %@", dict);
    /*		NSMenuItem *mi = [[NSMenuItem alloc] initWithTitle:[dict objectForKey:@"name"]
       action:@selector(viewEngineInfo:) keyEquivalent:@""]; [mi setTarget:self]; [mi setImage:[dict
       objectForKey:@"icon"]]; [mi setRepresentedObject:dict]; [mi setTag:[[dict objectForKey:@"uid"] intValue]];
    ...
    if (hasMatch) {
      ...
      if (isPOST) [md setObject:postData forKey:@"postData"];
...
- (void)rebuildDiscoveredEnginesPopup {
  ...
  NSMenuItem *mi;
  while (dict = [engineEnumerator nextObject]) {
    mi = [[NSMenuItem alloc] initWithTitle:[dict objectForKey:@"name"] action:@selector(viewEngineInfo:)
  keyEquivalent:@""]; [mi setTarget:self]; [mi setImage:[dict objectForKey:@"icon"]]; [mi setRepresentedObject:dict];
  ...
  [dict objectForKey:@"name"], ([added boolValue] ? @" (already added)" : @"")] attributes:[NSDictionary
  dictionaryWithObjectsAndKeys: [NSFont menuFontOfSize:[NSFont systemFontSize]], NSFontAttributeName, nil]]
  mutableCopy]; [att appendAttributedString:[[NSAttributedString alloc] initWithString:[dict objectForKey:@"url"]
  attributes:[NSDictionary dictionaryWithObjectsAndKeys: [NSFont systemFontOfSize:[NSFont smallSystemFontSize]],
  NSFontAttributeName, [NSColor controlShadowColor], NSForegroundColorAttributeName, nil]]];
...
- (IBAction)viewEngineInfo:(id)sender {
  ...
  if ([dict objectForKey:@"added"])
    if (![[dict objectForKey:@"added"] boolValue]) addenabled = YES;
...
//	// NSLog(@"HTTP headers: %@", [request allHTTPHeaderFields]);
  if ([[[[request URL] description] componentsSeparatedByString:MagicWord] count] > 1) {
    // NSLog(@"Search URL: %@", [[[[request URL] description] componentsSeparatedByString:MagicWord]
componentsJoinedByString:@"%@"]);
  }
...
- (void)webView:(WebView *)sender windowScriptObjectAvailable:(WebScriptObject *)windowScriptObject {
  ...
  // Make wrapper object (the actual _sidebar shows up as a typeof function, breaking most scripts (like Mozilla's own)
  // so we need to wrap it).
  [windowScriptObject
    evaluateWebScript:
      @"window.sidebar = {addSearchEngine: function(x,y,z,c){ window._sidebar.addSearchEngine(x,y,z,c); } };"];
  //	NSLog(@"fixed");
}
...
- (void)startStuff {
  ...
  NSImage *cog = [[NSImage alloc]
    initByReferencingFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"cog-menu%@", cogSuffix]
                                                          ofType:@"tiff"]];
...
+ (NSString *)webScriptNameForSelector:(SEL)sel {
  ...
  if (sel == @selector(addMycroftSearchEngine:iconURL:name:category:)) name = @"addSearchEngine";
...
- (void)saveEngines {
  ...
  while (ed = [engineEnumerator nextObject]) {
    ...
    [toPrefs setObject:[vt reverseTransformedValue:[[MonocleController controller]
                                                     deducedColorForImage:[ed objectForKey:@"icon"]]]
                forKey:@"color"];
    ...
    if (isPOST) {
      ...
      while (key = [keyEnumerator nextObject]) {
        [parr addObject:[NSDictionary
                          dictionaryWithObjectsAndKeys:key, @"key", [postData objectForKey:key], @"value", nil]];
      }
...
- (void)addMycroftSearchEngine:(NSString *)mycroftURL
                       iconURL:(NSString *)iconURL
                          name:(NSString *)name
                      category:(NSString *)category {
  //	NSLog(@"add mycroft search engine... url: %@, icon url: %@, name: %@, category: %@", mycroftURL, iconURL, name,
  // category);
...
- (void)parserCalledBack:(NSArray *)collect {
  ...
  if (searchOpenTag) {
    ...
    NSArray *searchkids =
      [document itemsStartingAt:searchOpenTag];  // more liberal parsing. // kidItemsOf:searchOpenTag];
                                                 //		NSLog(@"kids of search tag: %@", searchkids);

    if ([[searchOpenTag attributes] objectForKey:@"name"]) seName = [[searchOpenTag attributes] objectForKey:@"name"];
    if ([[searchOpenTag attributes] objectForKey:@"action"]) url = [[searchOpenTag attributes] objectForKey:@"action"];
...
- (IBAction)addClicked:(id)sender {
  //	[[[[discoveredEnginesPopup selectedItem] image] TIFFRepresentation] writeToFile:[@"~/Desktop/addedicon.tiff"
  ...
  if (isPOST) {
    ...
    while (key = [keyEnumerator nextObject]) {
      [parr
        addObject:[NSDictionary dictionaryWithObjectsAndKeys:key, @"key", [postData objectForKey:key], @"value", nil]];
    }
...
- (IBAction)goToWebsite:(id)sender {
  ...
  if ([[str stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""])
    return;
  if (NSEqualRanges([str rangeOfCharacterFromSet:[NSCharacterSet characterSetWithCharactersInString:@"."]],
        NSMakeRange(NSNotFound, 0))) {
    str = [NSString stringWithFormat:@"www.%@.com", str];
  }
  if (!([str hasPrefix:@"http://"] || [str hasPrefix:@"https://"])) {
    str = [NSString
      stringWithFormat:@"http://%@%@", str, ([[str componentsSeparatedByString:@"/"] count] < 2) ? @"/" : @""];
  }
...
- (IBAction)showInstructions:(id)sender {
  [[webView mainFrame] loadRequest:[NSURLRequest requestWithURL:[NSURL fileURLWithPath:[[NSBundle mainBundle]
