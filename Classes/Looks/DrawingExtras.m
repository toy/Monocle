...
/** The following courtesy entirely of a kind soul on the CocoaDev wiki: <http://www.cocoadev.com/>. **/
...
+ (NSBezierPath *)_joc_bezierPathWithJaggedRoundedRectInRect:(NSRect)r a:(float)a b:(float)b spacing:(float)spacing {
  ...
  *lengths = length += *gaps = hypotf(lastX - points[3].x, points[3].y - lastY);
  ...
  // top horizontal rail
  if (hRailCount) [path _joc_jaggedLineToPoint:pntPtr[quarter] teeth:hRailCount width:step buffer:railPoints];
  // quad2
