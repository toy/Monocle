    ...
    if (effectKey) {
      ...
      if ([nextTarget isKindOfClass:[NSView class]]) {
        ...
        if (![nextView isHidden] ||
          ![_fadeViews objectForKey:key]) {  // we always hide the view before starting a fade effect
...
- (NSRect)frameForView:(NSView *)view atProgress:(float)progress {
  ...
  return NSMakeRect(originalRect.origin.x + ((destinationRect.origin.x - originalRect.origin.x) * progress),
