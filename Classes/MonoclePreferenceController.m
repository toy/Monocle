...
- (id)transformedValue:(id)value {
  //    NSLog(@"google: %@ yahoo: %@", [value valueForKeyPath:@"suggestFromGoogle"], [value
  //    valueForKeyPath:@"suggestFromYahoo"]);
...
- (id)transformedValue:(id)value {
  ...
  if (type) isPOST = ([type isEqualToString:@"POST"]);
...
+ (void)initialize {
  ...
  trans = [[[MonocleShouldShowCountryPopUpValueTransformer alloc] init] autorelease];
  ...
  [NSValueTransformer setValueTransformer:trans forName:@"MonocleShouldShowCountryPopUp"];

  [NSValueTransformer setValueTransformer:trans2 forName:@"MonocleIsPOSTEngine"];

  [NSValueTransformer setValueTransformer:[[[MonocleCheckBoxToShouldBodgeValueTransformer alloc] init] autorelease]
                                  forName:@"MonocleCheckBoxToShouldBodge"];
}
...
- (void)awakeFromNib {
  ...
  [udc addObserver:self forKeyPath:@"values.suggestFromGoogle" options:0 context:NULL];

  [udc addObserver:self forKeyPath:@"values.suggestFromYahoo" options:0 context:NULL];
...
- (IBAction)removeEngineAfterAsking:(id)sender {
  NSAlert *al = [NSAlert alertWithMessageText:[NSString stringWithFormat:@"Do you want to remove the \"%@\" engine?",
                                                        [engineController valueForKeyPath:@"selection.name"]]
                                defaultButton:@"Delete engine"
                              alternateButton:@"Cancel"
                                  otherButton:nil
                    informativeTextWithFormat:@"This can't be undone."];
  [al beginSheetModalForWindow:mainWindow
                 modalDelegate:self
                didEndSelector:@selector(removeEngineAfterAskingAlertDidEnd:returnCode:contextInfo:)
                   contextInfo:NULL];
}
...
- (NSArray *)callwordSequence {
  return ([callwordFormatField objectValue] == nil ? [NSArray arrayWithObjects:@":", @"#callword#", @" ", nil]
                                                   : [callwordFormatField objectValue]);
}
...
- (void)shortcutRecorder:(SRRecorderControl *)aRecorder keyComboDidChange:(KeyCombo)newKeyCombo {
  if (aRecorder == hotKeyRecorder) {
    //		NSLog(@"key combo did change! (%@)",
    // SRStringForCocoaModifierFlagsAndKeyCode(newKeyCombo.flags,newKeyCombo.code));
...
- (void)setupToolbar {
  ...
  NSImage *enginesIcon = [[[NSImage alloc]
    initByReferencingFile:[[NSBundle bundleForClass:[self class]] pathForResource:@"engines-icon"
                                                                           ofType:@"png"]] autorelease];
  NSImage *generalIcon = [[[NSImage alloc]
    initByReferencingFile:[[NSBundle bundleForClass:[self class]] pathForResource:@"general"
                                                                           ofType:@"tiff"]] autorelease];

  NSDictionary *choices = [NSDictionary
    dictionaryWithObjectsAndKeys:[NSDictionary dictionaryWithObjectsAndKeys:MonoclePreferenceEngines,
                                               @"label",
                                               enginesIcon,
                                               @"image",
                                               enginesView,
                                               @"view",
                                               nil],
    MonoclePreferenceEngines,
    [NSDictionary
      dictionaryWithObjectsAndKeys:@"General", @"label", generalIcon, @"image", appearanceView, @"view", nil],
    MonoclePreferenceAppearance,
    [NSDictionary dictionaryWithObjectsAndKeys:MonoclePreferenceSearching,
                  @"label",
                  appIcon,
                  @"image",
                  searchingView,
                  @"view",
                  nil],
    MonoclePreferenceSearching,
    nil];

  //	toolbarChoices = [[NSArray arrayWithObjects:MonoclePreferenceEngines, MonoclePreferenceAppearance,
  // NSToolbarFlexibleSpaceItemIdentifier, MonoclePreferenceHelpButton, nil] retain];
  toolbarChoices = [[NSArray
    arrayWithObjects:MonoclePreferenceEngines, MonoclePreferenceAppearance, MonoclePreferenceSearching, nil] retain];
  ...
  while (key = [choiceEnumerator nextObject]) {
    ...
    [viewHeightsM setObject:[NSNumber numberWithFloat:[[engineDict objectForKey:@"view"] frame].size.height]
                     forKey:key];
  }
  ...
  viewForToolbarChoice = [[NSDictionary dictionaryWithObjectsAndKeys:enginesView, MonoclePreferenceEngines,
  searchingView, MonoclePreferenceSearching, appearanceView, MonoclePreferenceAppearance, nil] retain];
  labelForToolbarChoice = [[NSDictionary dictionaryWithObjectsAndKeys:MonoclePreferenceEngines,
  MonoclePreferenceEngines, MonoclePreferenceSearching, MonoclePreferenceSearching, MonoclePreferenceAppearance,
  MonoclePreferenceAppearance, nil] retain];

  NSImage *enginesImage = [NSImage imageNamed:@"NSApplicationIcon"]; //[[[NSImage alloc]
  initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"PrefsEngineIcon" ofType:@"tiff"]] retain];
  ...
  /*viewHeights = [[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:[enginesView
    frame].size.height], MonoclePreferenceEngines, [NSNumber numberWithFloat:[searchingView frame].size.height],
    MonoclePreferenceSearching, [NSNumber numberWithFloat:[appearanceView frame].size.height],
    MonoclePreferenceAppearance, nil] retain];*/

  NSString *selectedIdentifier;
  if (!(selectedIdentifier =
          [[NSUserDefaults standardUserDefaults] objectForKey:MonoclePreferenceSelectedToolbarPanelDefaultKey])) {
...
- (void)changePanel:(id)sender {
  ...
  if (nil != sender) {
    ...
    NSRect contentRect =
      NSMakeRect(basePoint.x, basePoint.y, sizeOfToBeView.width, sizeOfToBeView.height); /* SCREEN COORDS */
    ...
    if (oldView != currentView) {
      ...
      // WORKAROUND: There is a bug in NSViewAnimation where the window frame will not be set to its final value.  We
      // could fix this by adjusting the window frame in -animationDidEnd:, but that would result in a bumpy animation
      // where the view and window would get out of sync.  Instead, we do a little trick here, where we set the window
      // frame to a non-integral value knowing this will defeat the problematic check in NSViewAnimation.  Because
      // NSViewAnimation always adjust the window frame to an integral value, this trick will be harmless.
      ...
      /*2008-01-11 18:27:37.398 Monocle[7467:10b] mainWindowNewScreenFrame: {{164.01, 304}, {516.99, 474}}, contentRect:
       * {{0, 0}, {517, 396}} */
      ...
      animation = [[NSViewAnimation alloc]
        initWithViewAnimations:[NSArray arrayWithObjects:[NSDictionary dictionaryWithObjectsAndKeys:mainWindow,
                                                                       NSViewAnimationTargetKey,
                                                                       [NSValue valueWithRect:mainWindowNewScreenFrame],
                                        ...
                                        [NSDictionary dictionaryWithObjectsAndKeys:oldView,
                                                      ...
                                                      [NSValue valueWithRect:[oldView frame]],
                                                      NSViewAnimationEndFrameKey,
                                                      nil],
                                        [NSDictionary dictionaryWithObjectsAndKeys:currentView,
                                                      NSViewAnimationTargetKey,
                                                      NSViewAnimationFadeInEffect,
                                                      NSViewAnimationEffectKey,
                                                      [NSValue valueWithRect:contentRect],
                                                      NSViewAnimationEndFrameKey,
                                                      nil],
                                        nil]];
      NSTimeInterval dur = [mainWindow animationResizeTime:mainWindowNewScreenFrame];
      if (([[[NSApplication sharedApplication] currentEvent] modifierFlags] & NSShiftKeyMask)) dur *= 5.0;
      [animation setDuration:dur];
      [animation setDelegate:self];
      [currentView setHidden:YES];

      [animation startAnimation];
      [[NSUserDefaults standardUserDefaults] setObject:[toolbar selectedItemIdentifier]
                                                forKey:MonoclePreferenceSelectedToolbarPanelDefaultKey];
    }
  }
#else
...
- (IBAction)cancelAddSheet:(id)sender {
  ...
  if ([(NSNumber *)[obj objectForKey:@"toBeAdded"] boolValue]) [engineController removeObjectAtArrangedObjectIndex:idx];
...
- (IBAction)statusItemStyleChanged:(id)sender {
  ...
  if (([[statusItemStyle selectedItem] tag] == 4000) &&
    ([[[udc values] valueForKey:@"SearchBarStyle"]
      isNotEqualTo:[[[statusItemStyle selectedItem] representedObject] objectForKey:@"title"]])) {
  ...
  [[udc values] setValue:[[[statusItemStyle selectedItem] representedObject] objectForKey:@"title"]
                  forKey:@"SearchBarStyle"];
  //	[def synchronize];
...
- (void)buildStyleMenu {
  ...
  while (style = [styleEnumerator nextObject]) {
    ...
    if ([selectedStyle isEqualToString:[style objectForKey:@"title"]]) selTag = tag;
...
- (void)setSearchHelpProviders:(NSArray *)provs {
  ...
  while (prov = [provEnumerator nextObject]) {
    ...
    if ([isEnabled boolValue]) [enabled addObject:identifier];
  }
