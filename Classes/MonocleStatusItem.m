...
- (void)createImages {
  ...
  [[NSGraphicsContext currentContext] saveGraphicsState]; {
    CTGradient *gradient = [CTGradient gradientWithBeginningColor:[c shadowWithLevel:0.4] endingColor:[c
highlightWithLevel:0.4]]; [circle addClip]; [gradient fillRect:circleRect angle:90]; } [[NSGraphicsContext
currentContext] restoreGraphicsState]; #else [c setFill]; [circle fill]; #endif
   */
...
#define MonocleUndocumentedInvertMenuIconColorsKey @"MonocleUndocumentedInvertMenuIconColorsKey"
  ...
  NSImage *offGrooveGlass = [MonocleGlassIconDrawing imageForSize:size
                                                      strokeColor:[NSColor colorWithCalibratedWhite:1.0 alpha:0.45]];
...
#define MonocleUndocumentedUseImagesForMenuKey @"MonocleUndocumentedUseImagesForMenuKey"
  ...
  if ([[NSUserDefaults standardUserDefaults] boolForKey:MonocleUndocumentedUseImagesForMenuKey]) {
    NSString *appSupp = [[[NSHomeDirectory() stringByAppendingPathComponent:@"Library"]
      stringByAppendingPathComponent:@"Application Support"] stringByAppendingPathComponent:@"Monocle"];

    BOOL isDir;

    NSString *onImagePath = [appSupp stringByAppendingPathComponent:@"selected.png"];

    if ([[NSFileManager defaultManager] fileExistsAtPath:onImagePath isDirectory:&isDir] && !isDir) {
      NSImage *onImage = [[NSImage alloc] initWithContentsOfFile:onImagePath];
      if (onImage != nil) selectedIcon = [onImage copy];
      [onImage release];
    }
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:onImagePath isDirectory:&isDir] && !isDir) {
      ...
      if (offImage != nil) icon = [offImage copy];
      [offImage release];
    }
  }
}
...
- (void)drawRect:(NSRect)rect {
  ...
  [im drawInRect:insetRect
        fromRect:NSMakeRect(0.0, 0.0, imSize.width, imSize.height)
...
- (void)animateSheet:(BOOL)showing {
  ...
  if (showing) [[NSApplication sharedApplication] activateIgnoringOtherApps:YES];
  ...
  animation = [[NSViewAnimation alloc]
    initWithViewAnimations:[NSArray arrayWithObject:[NSDictionary dictionaryWithObjectsAndKeys:w,
                                                                  NSViewAnimationTargetKey,
                                                                  [NSValue valueWithRect:f],
                                                                  NSViewAnimationStartFrameKey,
                                                                  [NSValue valueWithRect:t],
                                                                  NSViewAnimationEndFrameKey,
                                                                  (showing ? NSViewAnimationFadeInEffect
