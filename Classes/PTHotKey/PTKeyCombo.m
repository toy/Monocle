...
- (id)plistRepresentation {
  return [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:[self keyCode]],
                       @"keyCode",
                       [NSNumber numberWithInteger:[self modifiers]],
                       @"modifiers",
                       nil];
}
...
+ (NSString*)_stringForModifiers:(long)modifiers {
  static unichar modToChar[4][2] = {{cmdKey, kCommandUnicode},
...
+ (NSString*)_stringForKeyCode:(short)keyCode legacyKeyCodeMap:(NSDictionary*)dict {
  ...
  if (!str) str = [NSString stringWithFormat:@"%X", keyCode];
...
+ (NSString*)_stringForKeyCode:(short)keyCode {
  ...
  if ([[dict objectForKey:@"version"] intValue] <= 0) return [self _stringForKeyCode:keyCode legacyKeyCodeMap:dict];
...
- (NSUInteger)modifierMask {
  ...
  static NSUInteger modToChar[4][2] = {{cmdKey, NSCommandKeyMask},
