...
NSString *SRStringForCarbonModifierFlagsAndKeyCode(unsigned int flags, signed short keyCode) {
  return [NSString stringWithFormat:@"%@%@", SRStringForCarbonModifierFlags(flags), SRStringForKeyCode(keyCode)];
}
...
NSString *SRStringForCocoaModifierFlagsAndKeyCode(unsigned int flags, signed short keyCode) {
  return [NSString stringWithFormat:@"%@%@", SRStringForCocoaModifierFlags(flags), SRStringForKeyCode(keyCode)];
}
...
//----------------------------------------------------------
#ifdef SR_IS_USING_LEOPARD
NSString *SRCharacterForKeyCodeAndCocoaFlags(signed short keyCode, unsigned int cocoaFlags) {
  PUDNSLog(@"SRCharacterForKeyCodeAndCocoaFlags, keyCode: %hi, cocoaFlags: %u", keyCode, cocoaFlags);
...
#define FailWithNaiveString SRStringForKeyCode(keyCode)
  ...
  currentLayout = TISCopyCurrentKeyboardLayoutInputSource();

  const UCKeyboardLayout *uchrData = (const UCKeyboardLayout *)CFDataGetBytePtr(
    TISGetInputSourceProperty(currentLayout, kTISPropertyUnicodeKeyLayoutData));
...
double SRAnimationEaseInOut(double t) {
  // This function implements a sinusoidal ease-in/ease-out for t = 0 to 1.0.  T is scaled to represent the interval of
  // one full period of the sine function, and transposed to lie above the X axis.
  double x = ((sin((t * M_PI) - M_PI_2) + 1.0) / 2.0);
  //	NSLog(@"SRAnimationEaseInOut: %f. a: %f, b: %f, c: %f, d: %f, e: %f", t, (t * M_PI), ((t * M_PI) - M_PI_2), sin((t
  //* M_PI) - M_PI_2), (sin((t * M_PI) - M_PI_2) + 1.0), x);
  return x;
}
...
+ (NSImage *)supportingImageWithName:(NSString *)name {
  ...
  double scaleValue = [[NSScreen mainScreen] userSpaceScaleFactor];
  size = NSMakeSize(size.width * scaleValue, size.height * scaleValue);
  //	NSLog(@"size: %@", NSStringFromSize(size));

  NSCustomImageRep *customImageRep =
    [[[NSCustomImageRep alloc] initWithDrawSelector:NSSelectorFromString([NSString stringWithFormat:@"_draw%@:", name])
  ...
  [tiff writeToURL:[NSURL fileURLWithPath:[[NSString stringWithFormat:@"~/Desktop/m_%@.tiff", name]
  ...
  NSCustomImageRep *customImageRepQDRPL =
    [[NSCustomImageRep alloc] initWithDrawSelector:NSSelectorFromString([NSString stringWithFormat:@"_draw%@:", name])
                                          delegate:self];
  ...
  [tiffQDRPL writeToURL:[NSURL fileURLWithPath:[[NSString stringWithFormat:@"~/Desktop/m_QDRPL_%@.tiff", name]
                                                 stringByExpandingTildeInPath]]
             atomically:YES];
...
+ (void)_drawSRSnapback:(id)anNSCustomImageRep {
  ...
  [bp curveToPoint:MakeRelativePoint(0.8508247, 0.4836237)
     controlPoint1:MakeRelativePoint(0.4085750, 0.7226146)
     controlPoint2:MakeRelativePoint(0.8371143, 0.7491841)];
  [bp curveToPoint:MakeRelativePoint(0.5507195, 0.0530682)
     controlPoint1:MakeRelativePoint(0.8677834, 0.1545071)
     controlPoint2:MakeRelativePoint(0.5507195, 0.0530682)];
  [bp curveToPoint:MakeRelativePoint(0.7421721, 0.3391942)
     controlPoint1:MakeRelativePoint(0.5507195, 0.0530682)
     controlPoint2:MakeRelativePoint(0.7458685, 0.1913146)];
  [bp curveToPoint:MakeRelativePoint(0.4085750, 0.5154130)
     controlPoint1:MakeRelativePoint(0.7383412, 0.4930328)
     controlPoint2:MakeRelativePoint(0.4085750, 0.5154130)];
