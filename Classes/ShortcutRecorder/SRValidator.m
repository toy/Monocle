...
- (BOOL)isKeyCode:(signed short)keyCode andFlagsTaken:(unsigned int)flags error:(NSError **)error;
{
  ...
  if (delegate) {
    ...
    if ([delegate shortcutValidator:self
      ...
      if (error) {
        NSString *description = [NSString stringWithFormat:SRLoc(@"The key combination %@ can't be used!"),
                                          SRStringForCarbonModifierFlagsAndKeyCode(flags, keyCode)];
        NSString *recoverySuggestion =
          [NSString stringWithFormat:SRLoc(@"The key combination \"%@\" can't be used because %@."),
        ...
        NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:description,
  ...
  while ((globalHotKeyInfoDictionary = [globalHotKeysEnumerator nextObject])) {
    ...
    globalHotKeyCharCode =
      [(NSNumber *)[globalHotKeyInfoDictionary objectForKey:(NSString *)kHISymbolicHotKeyCode] unsignedShortValue];
    globalHotKeyUniChar =
      [[[NSString stringWithFormat:@"%C", globalHotKeyCharCode] uppercaseString] characterAtIndex:0];

    CFNumberGetValue((CFNumberRef)[globalHotKeyInfoDictionary objectForKey:(NSString *)kHISymbolicHotKeyModifiers],
      kCFNumberSInt32Type,
      &gobalHotKeyFlags);
    ...
    if ((globalHotKeyUniChar == localHotKeyUniChar) && (globalCommandMod == localCommandMod) &&
      ...
      if (error) {
        ...
        NSString *recoverySuggestion = [NSString
          stringWithFormat:
            SRLoc(
              @"The key combination \"%@\" can't be used because it's already used by a system-wide keyboard shortcut. (If you really want to use this key combination, most shortcuts can be changed in the Keyboard & Mouse panel in System Preferences.)"),
          SRReadableStringForCarbonModifierFlagsAndKeyCode(flags, keyCode)];
        NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:description,
                                               @"NSLocalizedDescriptionKey",
                                               recoverySuggestion,
  ...
  while ((menuItem = [menuItemsEnumerator nextObject])) {
    ...
    if ((menuItemKeyEquivalent = [menuItem keyEquivalent]) && (![menuItemKeyEquivalent isEqualToString:@""])) {
      ...
      if (([[menuItemKeyEquivalent uppercaseString] isEqualToString:localKeyString]) &&
        (menuItemCommandMod == localCommandMod) && (menuItemOptionMod == localOptionMod) &&
        (menuItemShiftMod == localShiftMod) && (menuItemCtrlMod == localCtrlMod)) {
        if (error) {
          NSString *description = [NSString stringWithFormat:SRLoc(@"The key combination %@ can't be used!"),
                                            SRStringForCarbonModifierFlagsAndKeyCode(flags, keyCode)];
          NSString *recoverySuggestion = [NSString
