        ...
        */
		...
		if (effectKey) {
			...
			if ([nextTarget isKindOfClass:[NSView class]]) {
				...
				if (![nextView isHidden] || ![_fadeViews objectForKey:key]) { // we always hide the view before starting a fade effect
				...
				NSView *superview = [nextView superview];
...
- (NSRect)frameForView:(NSView *)view atProgress:(float)progress
{
    ...
    return NSMakeRect(
                      originalRect.origin.x + ((destinationRect.origin.x - originalRect.origin.x) * progress), 
                      originalRect.origin.y + ((destinationRect.origin.y - originalRect.origin.y) * progress), 
