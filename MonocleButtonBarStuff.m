...
+ (CTGradient *)glassGradient {
	if (glassGradient == nil) {
		...
		glassGradient = [glassGradient addColorStop:[NSColor colorWithCalibratedWhite:((double)0xFD/(double)0xFF) alpha:1.0] atPosition:0.0];
		glassGradient = [glassGradient addColorStop:[NSColor colorWithCalibratedWhite:((double)0xF3/(double)0xFF) alpha:1.0] atPosition:(9.99/21.0)];
		glassGradient = [glassGradient addColorStop:[NSColor colorWithCalibratedWhite:((double)0xE6/(double)0xFF) alpha:1.0] atPosition:(10.0/21.0)];
		glassGradient = [glassGradient addColorStop:[NSColor colorWithCalibratedWhite:((double)0xE6/(double)0xFF) alpha:1.0] atPosition:1.0];
...
//	[[result TIFFRepresentation] writeToFile:[@"~/Desktop/lightversion.tiff" stringByExpandingTildeInPath] atomically:YES];
...
+ (void)drawButtonBackgroundInRect:(NSRect)rect image:(NSImage *)image view:(NSView *)view {
	...
	[lighter drawAtPoint:NSMakePoint(startPoint.x, startPoint.y+((isFlipped ? 1.0 : -1.0)*lighterResolutionDependentOffset)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:fraction];
...
+ (NSImage *)buttonImageFromImage:(NSImage *)image {
	...
	[lighter drawAtPoint:NSMakePoint(startPoint.x, startPoint.y-1.0) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0];
	...
	//[tiff writeToURL:[NSURL fileURLWithPath:[[NSString stringWithFormat:@"~/Desktop/m_%@.tiff", @"add"] stringByExpandingTildeInPath]] atomically:YES];
