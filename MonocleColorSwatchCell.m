...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
	...
	if (![observedObjects containsObject:engine]) {
		[engine addObserver:self forKeyPath:@"icon" options:NSKeyValueObservingOptionNew context:MONOCLE_COLOR_SWATCH_CELL_MAGIC_BINDING_COOKIE];
		[engine addObserver:self forKeyPath:@"color" options:NSKeyValueObservingOptionNew context:MONOCLE_COLOR_SWATCH_CELL_MAGIC_BINDING_COOKIE];
		[engine addObserver:self forKeyPath:@"colorSpecified" options:NSKeyValueObservingOptionNew context:MONOCLE_COLOR_SWATCH_CELL_MAGIC_BINDING_COOKIE];
...
//	NSLog(@"color: %@", engineColor);
	...
	NSColor *shadowColor = [[([self isHighlighted] ? [NSColor selectedTextBackgroundColor] : [NSColor textBackgroundColor]) shadowWithLevel:0.35] contrastingLabelColor];
//	NSLog(@"highlighted: %@, color A: %@; contrasting: %@", ([self isHighlighted] ? @"Y" : @"N"), ([([self isHighlighted] ? [NSColor selectedTextBackgroundColor] : [NSColor textBackgroundColor]) shadowWithLevel:0.35]), shadowColor);
	...
	NSImage *pillImage = [[[NSImage alloc] initWithSize:NSMakeSize(width+padding+padding, height+padding+padding)] autorelease];
	
//	NSLog(@"width: %f, height: %f, lowest: %f", width, height, lowest);
	
//	NSRect r = NSMakeRect(NSMinX(cellFrame)+(lowest*0.4),NSMinY(cellFrame)+(NSHeight(cellFrame)-lowest)/2.0,width,lowest);
...
//	NSLog(@"rect: %@", NSStringFromRect(r));
	...
	CTGradient *grad = [CTGradient gradientWithBeginningColor:[engineColor highlightWithLevel:0.05] endingColor:[engineColor shadowWithLevel:0.05]];
	...
	CTGradient *whiteIntoTransp = [CTGradient gradientWithBeginningColor:engineColor endingColor:[NSColor colorWithCalibratedWhite:1.0 alpha:0.0]];
	whiteIntoTransp = [whiteIntoTransp addColorStop:engineColor atPosition:0.7];
	...
	[pill drawAtPoint:NSMakePoint(NSMinX(cellFrame), NSMinY(cellFrame)+(NSHeight(cellFrame)-[pill size].height)/2.0-(padding/2.0)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0];
...
/*	
	...
	[im drawAtPoint:NSMakePoint(NSMinX(cellFrame)+NSWidth(cellFrame)-iconWidth, NSMinY(cellFrame)+((NSHeight(cellFrame)-iconWidth)/2.0)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0];	
