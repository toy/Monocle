...
- (void)prepareWindow:(NSWindow *)window forSpacesUsingCollectionBehavior:(NSUInteger)mode {
	...
	@try {
		NSInvocation *setSpacesBehaviorInvocation = [NSInvocation invocationWithMethodSignature:[self methodSignatureForSelector:@selector(selectorForNSWindow_setCollectionBehavior:)]];
...
- (void)awakeFromNib {
	...
	NSWindow *whiteWindow = [[NSWindow alloc] initWithContentRect:NSMakeRect(0, screenSize.height*0.25, screenSize.width, screenSize.height*0.75) styleMask:NSBorderlessWindowMask backing:NSBackingStoreBuffered defer:NO];
...
//	[whiteWindow orderFront:self];
	...
	searchWindow = [[MonocleSearchWindow alloc] initWithContentRect:NSMakeRect(-5000, -5000, NSWidth(searchRect), NSHeight(searchRect))
...
//	NSUInteger collectionTransient = (1 << 3); // NSWindowCollectionBehaviorTransient
	...
	[self prepareWindow:searchWindow forSpacesUsingCollectionBehavior:NSWindowCollectionBehaviorMoveToActiveSpace | NSWindowCollectionBehaviorTransient];//(collectionCanJoinAllSpaces | collectionTransient)];
...
//		NSLog(@"candidate: %@", fileName);
		if ([[fileName pathExtension] isEqualToString:@"monoclePlugin"]) {
			if ([fileName hasPrefix:@"."]) {
				NSLog(@"INFO! A hidden Monocle plugin, '%@', was found; it was not loaded. Make it visible (remove the dot prefix) to load it.", fileName);
				continue;
			}
...
- (void)registerDefaultSearchEngines {
	...
	NSImage *google = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Google" ofType:@"tiff"]];
	NSImage *wikipedia = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Wikipedia" ofType:@"tiff"]];
	NSImage *bing = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Bing" ofType:@"tiff"]];
	NSImage *youTube = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"YouTube" ofType:@"tiff"]];
	NSImage *yahoo = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Yahoo" ofType:@"tiff"]];
	
	NSArray *defaultSearchEngines = [NSArray arrayWithObjects:
		[NSDictionary dictionaryWithObjectsAndKeys:
			@"Google", @"name",
			[vt reverseTransformedValue:[google autorelease]], @"icon",
			[vt reverseTransformedValue:[self deducedColorForImage:google]], @"color",
			@"Google Web Search", @"extraInfo",
			@"http://www.google.com/search?ie=utf8&oe=utf8&q=%@", @"get_URL",
			@"g", @"callword",
			@"UTF-8", @"encoding",
			@"GET", @"type",
			nil],
		[NSDictionary dictionaryWithObjectsAndKeys:
			@"Wikipedia", @"name",
			[vt reverseTransformedValue:[wikipedia autorelease]], @"icon",
			[vt reverseTransformedValue:[self deducedColorForImage:wikipedia]], @"color",
			@"The open encyclopedia", @"extraInfo",
			@"http://en.wikipedia.org/wiki/Special:Search?search=%@&go=Go", @"get_URL",
//			@"underscore", @"get_transmutation",
		...
		[NSDictionary dictionaryWithObjectsAndKeys:
			@"Bing", @"name",
			[vt reverseTransformedValue:[bing autorelease]], @"icon",
			[vt reverseTransformedValue:[self deducedColorForImage:bing]], @"color",
			@"Bing Web Search", @"extraInfo",
			@"http://www.bing.com/search?q=%@", @"get_URL",
			@"b", @"callword",
			@"UTF-8", @"encoding",
			@"GET", @"type",
			nil],		
		[NSDictionary dictionaryWithObjectsAndKeys:
			@"Yahoo!", @"name",
			[vt reverseTransformedValue:[yahoo autorelease]], @"icon",
			[vt reverseTransformedValue:[self deducedColorForImage:yahoo]], @"color",
			@"Yahoo! Search", @"extraInfo",
			@"http://search.yahoo.com/search?p=%@&ei=UTF-8", @"get_URL",
			@"y", @"callword",
			@"UTF-8", @"encoding",
			@"GET", @"type",
			nil],
		[NSDictionary dictionaryWithObjectsAndKeys:
			@"YouTube", @"name",
			[vt reverseTransformedValue:[youTube autorelease]], @"icon",
			[vt reverseTransformedValue:[self deducedColorForImage:youTube]], @"color",
			@"YouTube Video Search", @"extraInfo",
			@"http://www.youtube.com/results?search_query=%@", @"get_URL",
			@"yt", @"callword",
			@"UTF-8", @"encoding",	
			@"GET", @"type",
			nil],
	...
	[MonoclePreferences registerDefaultPreferences:[NSDictionary dictionaryWithObjectsAndKeys:
		defaultSearchEngines, @"engines",
													[NSNumber numberWithBool:YES], @"emptiesSearchFieldBetweenSearches",
		nil]
		];
}
...
- (void)dispatchGETSearchForQuery:(NSString *)query usingEngine:(id)engine {
	...
	if (!getURL) return;
	if ([[getURL stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""]) return;
...
//	NSLog(@"POST: GET URL: %@", getURL);
	...
	NSString *formHTML = [NSString stringWithFormat:@"<html><head><title>Monocle POST search for \"%@\"</title><style>%@</style></head><body><form action=\"%@\" id=\"post-form\" accept-charset=\"%@\" method=\"POST\" target=\"_self\">", [query stringByHTMLEntityEscaping], MonoclePOSTSearchPageStyle, urlToLaunch, iana];
...
//		NSLog(@"kv: %@", kv);
		id k = [[[[kv valueForKey:@"key"] componentsSeparatedByString:@"%@"] componentsJoinedByString:[query stringByHTMLEntityEscaping]] stringByReplacingPercentEscapesUsingEncoding:enc];
		id v = [[[[kv valueForKey:@"value"] componentsSeparatedByString:@"%@"] componentsJoinedByString:[query stringByHTMLEntityEscaping]] stringByReplacingPercentEscapesUsingEncoding:enc];
		formHTML = [formHTML stringByAppendingFormat:@"<input type=\"hidden\" name=\"%@\" value=\"%@\" />", k, v];
	} 

	NSFileManager *fm = [NSFileManager defaultManager]; BOOL isDir;
	...
	if (im) {
		...
		[bit autorelease];
		NSString *iconpath = [tmpFolder stringByAppendingPathComponent:[NSString stringWithFormat:@"%i.png", [engine hash]]];
		[png writeToFile:iconpath atomically:YES];
		iconhtml = [NSString stringWithFormat:@"<img src=\"%@\" alt=\"%@\" /> ", [[NSURL fileURLWithPath:iconpath] absoluteString], [[engine valueForKey:@"name"] stringByHTMLEntityEscaping]];
	}
	
	formHTML = [formHTML stringByAppendingFormat:@"<script>document.getElementById('post-form').submit();</script><div>Monocle search using %@<b>%@</b>: <input type=\"submit\" value=\"Perform the search for '%@'\" /></div></form></body></html>", iconhtml, [[engine valueForKey:@"name"] stringByHTMLEntityEscaping], [query stringByHTMLEntityEscaping]];
	
	NSString *tmpFile = [tmpFolder stringByAppendingPathComponent:[NSString stringWithFormat:@"monocle-tmp-post-search-%i.html", [formHTML hash]]];
...
- (NSBitmapImageRep *)bitmapImageRepForImage:(NSImage *)currIcon {
	...
	if ([rep isKindOfClass:[NSBitmapImageRep class]])
		...
		// Make really sure we get a bitmap rep by making NSBitmapImageRep render a tiff of our rep and then init a new bitmap rep from that
...
- (NSArray *)statusItemStyles {
	return [NSArray arrayWithObjects:
		[NSDictionary dictionaryWithObjectsAndKeys:[self customStatusItemColor], @"color", @"Custom", @"title", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient gradientWithBeginningColor:GRADIENT_LEOPARD_START endingColor:GRADIENT_LEOPARD_END], @"gradient", @"Leopard", @"title", @"YES", @"start", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient gradientWithBeginningColor:GRADIENT_LEOPARD_PALE_START endingColor:GRADIENT_LEOPARD_PALE_END], @"gradient", @"Leopard Pale", @"title", @"YES", @"fallback", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[[NSColor blackColor] highlightWithLevel:0.2], @"color", @"Charcoal", @"title", @"YES", @"start", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[NSColor alternateSelectedControlColor], @"color", @"Selection", @"title", nil],
/*		[NSDictionary dictionaryWithObjectsAndKeys:[NSColor colorForControlTint:NSBlueControlTint], @"color", @"Blue", @"title", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[NSColor colorForControlTint:NSGraphiteControlTint], @"color", @"Graphite", @"title", nil],*/

/*		[NSDictionary dictionaryWithObjectsAndKeys:[[NSColor whiteColor] shadowWithLevel:0.2], @"color", @"Off-White", @"title", nil],*/
		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient aquaNormalGradient], @"gradient", @"Aqua", @"title", @"YES", @"no-sheen", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient aquaSelectedGradient], @"gradient", @"Aqua Selected", @"title", @"YES", @"no-sheen", nil],
/*		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient unifiedNormalGradient], @"gradient", @"Unified", @"title", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient unifiedSelectedGradient], @"gradient", @"Unified Selected", @"title", nil],
		[NSDictionary dictionaryWithObjectsAndKeys:[CTGradient unifiedDarkGradient], @"gradient", @"Unified Dark", @"title", nil],*/
		nil];
}
...
- (NSColor *)customStatusItemColor {
	...
	if (nil != i)
		c = (NSColor *)[NSUnarchiver unarchiveObjectWithData:i];
...
- (IBAction)fixGlobalHotKey:(id)sender {
	...
	if (sender) {
		...
		ptkc = [PTKeyCombo keyComboWithKeyCode:keyCode
									 modifiers:SRCocoaToCarbonFlags(flags)];
...
- (id)initWithFrame:(NSRect)frame {
    ...
    if (self) {
		...
		[nc addObserver:self
			   selector:@selector(refreshWashView:)
