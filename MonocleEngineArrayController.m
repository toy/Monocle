...
//		NSLog(@"selection changed");
	...
	} else if ([keyPath isEqualToString:@"color"] || [keyPath isEqualToString:@"icon"] || [keyPath isEqualToString:@"colorSpecified"]) {
...
- (BOOL)canRemove {
	if ((![self arrangedObjects]) || ([[self arrangedObjects] count] < 2))
		return NO;
	...
	dropOperation:(NSTableViewDropOperation)op
{
    ...
    if ([[[info draggingPasteboard] types] containsObject:@"MonocleIntrisicTableEngineType"]) {
		...
		NSIndexSet *indexSet = [trf transformedValue:[[info draggingPasteboard] propertyListForType:@"MonocleIntrisicTableEngineType"]];
