...
+ (NSImage *)imageForSize:(NSSize)s strokeColor:(NSColor *)strokeColor {
  ...
  // x is the thickness of the strokes, and in Spotlight's icon it's about 7% of the height (1.5 thick in a 22 high
  // icon). Given a height of 10, where x is 1.5, these are approximate proportions
  //   __
  ...
  //	NSLog(@"s: %@; lensTotalSize: %f; lensOrigin: %@", NSStringFromSize(s), lensTotalSize,
  // NSStringFromPoint(lensOrigin));

  NSAffineTransform *transform = [NSAffineTransform transform];
  // The original implementation used ((s.height / 22) * 10) / 2 as the circle's origin.
  [transform translateXBy:lensOrigin.x yBy:lensOrigin.y];
  ...
  [handle moveToPoint:NSMakePoint(handleoffset * widthMultiplier, handleoffset * heightMultiplier)];
  [handle relativeLineToPoint:NSMakePoint(handlelength * widthMultiplier, handlelength * heightMultiplier)];
  ...
  [moveHandleIntoPlaceTransform translateXBy:NSMidX(circleRect) yBy:NSMidY(circleRect)];
