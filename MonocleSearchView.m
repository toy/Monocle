...
- (void)awakeFromNib {
	...
	[selectedEngine bind:@"contentObject" toObject:self withKeyPath:@"selectedEngine" options:
	...
	borderlessSearchHelpWindow = [[NSWindow alloc] initWithContentRect:NSMakeRect(-5000, -5000, NSWidth(webframe), NSHeight(webframe)) styleMask:NSBorderlessWindowMask
	...
	[[searchHelpWebView mainFrame] loadRequest:[NSURLRequest requestWithURL:[NSURL fileURLWithPath:[[NSBundle bundleForClass:[self class]] pathForResource:@"results" ofType:@"html"]]]];
	...
	[selectedEngine addObserver:self
					 forKeyPath:@"selection"
						options:NSKeyValueObservingOptionNew context:@"selectedEngine"];

	[[NSUserDefaultsController sharedUserDefaultsController] addObserver:self
						forKeyPath:@"values.engines"
						   options:NSKeyValueObservingOptionNew context:@"allEngines"];
}
...
//	NSValueTransformer *trf = [NSValueTransformer valueTransformerForName:NSUnarchiveFromDataTransformerName];
	...
	while ((engine = [enumerator nextObject]) != nil) {
		if ([[engine valueForKey:@"toBeAdded"] boolValue]) {idx++; continue; }
		...
		NSMenuItem *mi = [[[NSMenuItem alloc] initWithTitle:tengineName action:@selector(changeEngineToMenuItem:) keyEquivalent:@""] autorelease];
		[mi setTarget:self];
		[mi setTag:idx];
...
- (void)bringUp {
	...
	[self whenSelectedEngineChanges];
	[[[textField window] fieldEditor:YES forObject:textField] setSelectedRange:NSMakeRange(0,[[textField stringValue] length])];
}
...
#define MonocleSearchViewHitTestRightCap			(MonocleSearchViewDrawingSettingsWidth+MonocleSearchViewDrawingSettingsOffset)*1.1
...
//	NSRect rightCapHitFrame = NSMakeRect(NSMaxX([self frame])-MonocleSearchViewHitTestRightCap,0.0,MonocleSearchViewHitTestRightCap,NSHeight([self frame]));
//	NSLog(@"width of left hitFrame: %f, location: %f", MonocleSearchViewHitTestLeftCap, local_point.x);
//	NSLog(@"width of right hitFrame: %f, rect: %@, location: %f", MonocleSearchViewHitTestRightCap, NSStringFromRect(rightCapHitFrame), local_point.x);
...
- (void)doPopUpMenu {
	NSRect popUpCellFrame = NSMakeRect(NSMaxX([self frame])-MonocleSearchViewHitTestRightCap,0.0,MonocleSearchViewHitTestRightCap,NSHeight([self frame]));
...
- (void)drawRect:(NSRect)rect {
	...
	if (asked.width < whole.width || asked.height < whole.height) { [self display]; return; }
	...
	* draw right end arrow (sharp shadow) */
	...
	double edgePaddingLeftCap = (height - MonocleSearchViewDrawingIconSize)*0.4;//MonocleSearchViewDrawingIconSize*0.3;
	double widthOfLeftCapArea = widthOfLeftCapInternals+(edgePaddingLeftCap*2.0);
	NSPoint iconOriginInLeftCap = NSMakePoint(edgePaddingLeftCap, NSMidY(rect)-(MonocleSearchViewDrawingIconSize/2.0));
	...
	NSPoint arrowOriginInRightCap = NSMakePoint(NSMaxX(rect)-((widthOfRightCapArea+settingsArrowSize.width)/2.0), NSMidY(rect)-(settingsArrowSize.height/2.0));
...
//	NSLog(@"enginecolor: %@", engineColor);
	...
	[moveDownArrow translateXBy:arrowOriginInLeftCap.x+(offsetBetweenArrowsAndIconEdgeInLeftCap.width) yBy:arrowOriginInLeftCap.y+(offsetBetweenArrowsAndIconEdgeInLeftCap.height/2.0)];
...
- (void)showNextPrev {
	...
	NSRect newRect = NSMakeRect(NSMinX(existRect),NSMaxY(existRect)-(baseHeight*2.0),NSWidth(existRect),baseHeight*3.0);
...
//		NSLog(@"doesn't contain rect");
		...
		if (origRnewRect.origin.y < 0.0)
			rnewRect.origin.y = 0.0;
		...
		if (origRnewRect.origin.x < 0.0)
			rnewRect.origin.x = 0.0;
	}
	...
	NSMutableDictionary *md = [[NSDictionary dictionaryWithObjectsAndKeys:
		[self window], NSViewAnimationTargetKey,
		[NSValue valueWithRect:rnewRect], NSViewAnimationEndFrameKey,
		nil] mutableCopy];
	NSLog(@"rnewRect: %@", NSStringFromRect(rnewRect));

	NSDictionary *textFieldTransformation = [NSDictionary dictionaryWithObjectsAndKeys:
		textField, NSViewAnimationTargetKey,
...
- (void)hideNextPrev {
	...
	NSRect newRect = NSMakeRect(NSMinX(existRect),NSMaxY(existRect)+(baseHeight*-2.0),NSWidth(existRect),baseHeight);
	NSRect rnewRect = [[self window] contentRectForFrameRect:newRect];
	NSRect origRnewRect = rnewRect;
...
- (IBAction)doSearch:(id)sender {
	if ([[[self searchText] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualTo:@""]) return;
...
//				NSLog(@"adding name: %@, callword: %@", [engine objectForKey:@"name"], [engine objectForKey:@"callword"]);
...
//	NSLog(@"text is: %@", text);
	...
	if ((prefix == nil || [text hasPrefix:prefix]) && [callwords count] > 0) {
		...
		while (key = [keyEnumerator nextObject]) {
			...
			NSString *lookFor = ((prefix == nil) ? [NSString stringWithFormat:@"%@ ", cw] : [NSString stringWithFormat:@"%@%@ ", prefix, cw]);
			if ([text hasPrefix:lookFor]) {
				...
				int i = 0;
				while (engine = [engineEnumerator nextObject]) {
					if ([[engine objectForKey:@"name"] isEqualToString:key])
						break;
					i++;
				} 
	...
	[mutableText replaceOccurrencesOfString:@"\t" withString:@" " options:NSLiteralSearch range:NSMakeRange(0, [mutableText length])];
	[mutableText replaceOccurrencesOfString:@"\r\n" withString:@" " options:NSLiteralSearch range:NSMakeRange(0, [mutableText length])];
	[mutableText replaceOccurrencesOfString:@"\n" withString:@" " options:NSLiteralSearch range:NSMakeRange(0, [mutableText length])];
	[mutableText replaceOccurrencesOfString:@"\r" withString:@" " options:NSLiteralSearch range:NSMakeRange(0, [mutableText length])];
...
- (void)updateSearchHelpTimer:(NSTimer *)timer {
	...
	if (([string isEqualToString:currentSearchhelpIsForString] || [string isEqualToString:draftSearchhelpIsForString]) && !wantsToUpdateSearchhelp) {
...
//			NSLog(@"[%d] assembling list (%@)", job, [draftSearchhelpResults allKeys]);
			...
			NSString *orderedId; while (orderedId = [orderedEnumerator nextObject]) {
...
- (BOOL)updateSearchHelp:(NSString *)string {
	...
	@synchronized(searchhelpHandle) {
		...
		[MonocleSuggestionProvider combinedResultsSuggestionsForString:string forJob:currentSearchhelpJob usingEngine:[self selectedEngine] delegate:self];
...
- (BOOL)hasNonEmptySearchHelpSelected {
	BOOL x = [[[searchHelpWebView windowScriptObject] callWebScriptMethod:@"hasAnySelection" withArguments:[NSArray array]] boolValue];
//	NSLog(@"hasNonEmptySearchHelpSelected? %d", x);
	return x;
}

- (void)doTheRightThingWhenSearchHelpSelectionChanges {
	id selection = [[searchHelpWebView windowScriptObject] callWebScriptMethod:@"getSelection" withArguments:[NSArray array]];
...
- (BOOL)fillInSearchHelp {
	...
	if ([selectedSearchHelp objectForKey:@"suggestion"]) {
		...
		if ([[sugg lowercaseString] hasPrefix:[currentText lowercaseString]]) {
			...
			[[[textField window] fieldEditor:NO forObject:textField] setSelectedRange:NSMakeRange(width, [sugg length]-width)];
		} else {
			[[[textField window] fieldEditor:NO forObject:textField] setSelectedRange:NSMakeRange(0, [sugg length])];
		}*/
		
		[textField setStringValue:sugg];
		[self controlTextDidChange:[NSNotification notificationWithName:NSControlTextDidChangeNotification object:textField]];
...
//		NSLog(@"supposed to navigate to site: %@", selectedSearchHelp);
		BOOL succeeded = [MonocleSuggestionProvider openResult:[selectedSearchHelp objectForKey:@"url"] usingProviderWithIdentifier:[selectedSearchHelp objectForKey:@"providerIdentifier"]];
...
- (BOOL)showsSearchHelp { return anySearchHelp; }
...
//		NSLog(@"resultsJSON: %@", resultsJSON);
		...
		[[searchHelpWebView windowScriptObject] callWebScriptMethod:@"updateResultsList" withArguments:[NSArray arrayWithObjects:resultsJSON, [self searchText], [[self searchText] lowercaseString], nil]];
...
- (void)nilOutSearchHelp {
	currentSearchhelpJob++;
	[[searchHelpWebView windowScriptObject] callWebScriptMethod:@"updateResultsList" withArguments:[NSArray arrayWithObjects:[[NSDictionary dictionaryWithObject:[NSArray array] forKey:@"results"] jsonStringValue], @"", @"", nil]];
...
- (void)showOrHideSearchHelp {
	if (!isSearchViewShown) {
		[self hideSearchHelp]; return;
	}
	if (!hasSetSpacesBehavior) {
		[controller prepareWindow:borderlessSearchHelpWindow forSpacesUsingCollectionBehavior:1 /* NSWindowCollectionBehaviorCanJoinAllSpaces */];	
	...
	BOOL noResults = [[[searchHelpWebView windowScriptObject] callWebScriptMethod:@"isEmpty" withArguments:[NSArray array]] boolValue];
	if (noResults) {
		[self hideSearchHelp];
	} else {
		anySearchHelp = YES;
...
//		NSLog(@"calc height: %f", calcHeight);
		...
		//		[[searchHelpWebView window] setFrame:[[searchHelpWebView window] frameRectForContentRect:NSMakeRect(NSMinX(searchPanelFrame), y - calcHeight, NSWidth(searchPanelFrame), calcHeight)] display:YES];//MAX(NSHeight(webRect), 120))];
		//		[[searchHelpWebView window]  setFrameTopLeftPoint:NSMakePoint(NSMinX(searchPanelFrame), NSMinY(searchPanelFrame))];
	}	
}
...
+ (BOOL)isSelectorExcludedFromWebScript:(SEL)aSelector {
	return (aSelector != @selector(jsvisible_goToResult:) && aSelector != @selector(jsvisible_getHeight) && aSelector != @selector(jsvisible_getInstructionsInnerHTML) && aSelector != @selector(jsvisible_getInstructionsInnerHTMLNonSelection));
}
...
- (NSString *)jsvisible_getInstructionsInnerHTML {
	...
	return [NSString stringWithFormat:
			NSLocalizedString(@"Search with %@. Follow suggestion/result with %@.", @"Instructions in search help (selection active)"),
...
//	BOOL enterMeansSearch = [MonoclePreferences boolForKey:@"searchTakesPriority" orDefault:NO];
	...
	return [NSString stringWithFormat:
			NSLocalizedString(@"Search with %@. Select suggestion/result with %@/%@.", @"Instructions in search help"),
			//			[engineName stringByHTMLEntityEscaping],
...
//		subtle, NSShadowAttributeName,
	...
	if ([data isKindOfClass:[MonocleResult class]]) {
		...
		[mas appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@\n",[res title]] attributes:bolded]];
		if ([res description] != nil) {
			[mas appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@\n",[res description]] attributes:descr]];
		}
		[mas appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@\n",[res location]] attributes:loc]];
	} else if ([data isKindOfClass:[MonocleSuggestion class]]) {
		MonocleSuggestion *sug = data;
...
- (double)searchhelpHeightForRow:(int)rowIndex {
	...
	NSTextContainer *textContainer = [[[NSTextContainer alloc]
        initWithContainerSize: NSMakeSize([[[searchHelpTable tableColumns] objectAtIndex:0] width], FLT_MAX)] autorelease];
...
- (void)tableView:(NSTableView *)aTableView willDisplayCell:(id)aCell forTableColumn:(NSTableColumn *)aTableColumn row:(int)rowIndex {
	...
	if ([data isKindOfClass:[NSString class]]) {
		...
		[mas addAttribute:NSBackgroundColorAttributeName value:searchhelpTableGridColor range:NSMakeRange(0,[[mas string] length])];
		[c setBackgroundColor:searchhelpTableGridColor];
		[c setAttributedStringValue:mas];
	} else {
		[c setDrawsBackground:NO];
		NSAttributedString *as = [self searchhelpAttributedStringForRow:rowIndex];
...
/*	NSLog(@"rect: %@ for size: %@ of row: %@", 
		  NSStringFromRect([as boundingRectWithSize:NSMakeSize(NSWidth([tableView frame]),90000.0) options:(NSStringDrawingUsesDeviceMetrics & NSStringDrawingUsesFontLeading)]), 
