...
- (NSString *)stringByResolvingJavascriptEscapes {
	...
	NSCharacterSet *simpleEscapes = [NSCharacterSet characterSetWithCharactersInString:@"\\\"'bfnrt"]; // \, ", ', [bfnrt]
	NSDictionary *simpleEscapeMap = [NSDictionary dictionaryWithObjectsAndKeys:@"\\", @"\\",  @"\"", @"\"",  @"'", @"'",  @"\b", @"b",  @"\f", @"f",
									 @"\n", @"n",   @"\r", @"r",   @"\t", @"t", nil];
	do {
		[scannerForMe scanUpToString:@"\\" intoString:&str];
		if (str != nil) {
			[stringToKeep appendString:str];
			if ([scannerForMe isAtEnd])
				break;
...
+ (void)initialize {
	if (self == [MonocleSuggestionProvider class]) {
		...
		[NSTimer scheduledTimerWithTimeInterval:0.02 target:self selector:@selector(registerKnownVendors:)
...
//	NSLog(@"Registering provider vendor class: %@", NSStringFromClass(providerVendor));
	...
	NSAssert1(([providerVendor respondsToSelector:@selector(providesResults)] && [providerVendor respondsToSelector:@selector(providesSuggestions)]),
			  @"ERROR! The class %@ is not a valid provider vendor.", NSStringFromClass(providerVendor));
	NSString *vendorIdentifier = [providerVendor vendorIdentifier];
	if ([vendorIdentifiers containsObject:vendorIdentifier]) {
		NSLog(@"ERROR! Class %@ tried to register the provider vendor identifier %@ a second time.", NSStringFromClass(providerVendor), vendorIdentifier);
		return;
...
/*	NSAssert1(([providerVendor vendorIdentifier] != nil),
			  @"The class %@ is a valid provider vendor but does not provide a valid vendorIdentifier.", NSStringFromClass(providerVendor));*/
	if ([providerVendor providesResults]) {
		id<MonocleResultProviding> rp = [providerVendor resultProvider];
		if (rp == nil) {
			NSLog(@"ERROR! Class %@ claims to support providing results, but provides a nil result provider.", NSStringFromClass(providerVendor));
			return;
		}
	...
	if ([providerVendor providesSuggestions]) {
		...
		if (sp == nil) {
			NSLog(@"ERROR! Class %@ claims to support providing suggestions, but provides a nil suggestion provider.", NSStringFromClass(providerVendor));
			return;
		}		
...
+ (NSArray *)orderedEnabledIdentifiers {
	...
	NSArray *enabledProviders = [MonoclePreferences arrayForKey:@"SearchHelpProvidersEnabled" orDefault:providerIdentifiers];
	...
	while (identifier = [helperEnumerator nextObject]) {
		if ([enabledProviders containsObject:identifier])
...
+ (NSArray *)orderedEnabledIdentifiersForEngine:(id)engine {
	...
	NSArray *defaultProviders = [MonoclePreferences arrayForKey:@"SearchHelpProvidersEnabled" orDefault:providerIdentifiers];
...
//	NSLog(@"[%d] %@ found out %d results for %@", job, [res resultsSourceIdentifier], [results count], string);
	[delegate resultsSuggestions:[NSArray arrayWithObjects:[res resultsSource], [res resultsSourceIdentifier], results, nil] fromProvider:[res resultsSourceIdentifier] forJob:job];
...
//	NSLog(@"[%d] %@ found out %d suggestions for %@", job, [sugg suggestionsSourceIdentifier], [suggs count], string);
	[delegate resultsSuggestions:[NSArray arrayWithObjects:[sugg suggestionsSource], [sugg suggestionsSourceIdentifier], suggs, nil] fromProvider:[sugg suggestionsSourceIdentifier] forJob:job];
...
+ (NSArray *)resultsForString:(NSString *)string fromResultProvider:(id<MonocleResultProviding>)res {
	...
	NSDictionary *tcr = [NSDictionary dictionaryWithObjectsAndKeys:
						 results, @"cache",
						 [[NSDate date] addTimeInterval:MONOCLERESULTCACHETIMEOUT], @"expire",
						 nil];
...
+ (NSArray *)suggestionsForString:(NSString *)string fromSuggestionProvider:(id<MonocleSuggestionProviding>)sugg {
	...
	NSDictionary *tcs = [NSDictionary dictionaryWithObjectsAndKeys:
						 suggestions, @"cache",
						 [[NSDate date] addTimeInterval:MONOCLERESULTCACHETIMEOUT], @"expire",
						 nil];
...
+ (void)combinedResultsSuggestionsForString:(NSString *)string forJob:(unsigned int)job usingEngine:(id)engine delegate:(id)delegate {
	...
	NSDictionary *context = [NSDictionary dictionaryWithObjectsAndKeys:
							 [NSNumber numberWithUnsignedInt:job], @"job",
							 string, @"string",
							 delegate, @"delegate",
							 nil];
...
+ (id<MonocleSuggestionProviding>)suggestionProvider { return nil; }
+ (id<MonocleResultProviding>)resultProvider { return nil; }
...
+ (id<MonocleSuggestionProviding>)suggestionProvider { return [self __sharedInstance]; }
...
- (NSArray *)suggestionsForString:(NSString *)string {
	...
	NSArray *sugg = [spc completionsForPartialWordRange:[string rangeOfString:string] inString:string language:nil inSpellDocumentWithTag:0];
...
+ (id<MonocleSuggestionProviding>)suggestionProvider { return [self sharedInstance]; }
+ (id<MonocleResultProviding>)resultProvider { return [self sharedInstance]; }
...
- (id) init
{
	...
	if (self != nil) {
		...
		NSString *suggestionExtractionScript = [NSString stringWithContentsOfFile:[[NSBundle bundleForClass:[self class]] pathForResource:@"GoogleSuggestExtract" ofType:@"js"] encoding:NSUTF8StringEncoding error:NULL];
		
		[[webView mainFrame] loadHTMLString:[NSString stringWithFormat:@"<script>%@</script>", suggestionExtractionScript] baseURL:nil];
	}
	return self;
}
...
- (void)runSuggestionsParsing:(NSString *)callToExtractSuggestions {
	WebScriptObject *wso = nil; BOOL looped = NO;
...
- (NSArray *)suggestionsForString:(NSString *)string {
	...
	NSData *resultData = [NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://www.google.com/complete/search?js=true&qu=%@", urlenc]]];
	NSString *result = [[NSString alloc] initWithData:resultData encoding:NSUTF8StringEncoding];
//	NSLog(@"result for %@:\n--\n%@\n--", string, result);
	
	// OLD VERSION window.google.ac.Suggest_apply(frameElement, "iphone", new Array(2, "iphone", "152,000,000 results", "iphone games", "188,000,000 results", "iphone apps", "54,600,000 results", "iphone unlock", "2,390,000 results", "iphone accessories", "4,660,000 results", "iphone review", "156,000,000 results", "iphone canada", "32,500,000 results", "iphone applications", "62,000,000 results", "iphone ringtones", "2,240,000 results", "iphones", "8,060,000 results"), new Array(""));
	...
	while (canContinue) {
		...
		if (res != nil)
			[results addObject:[MonocleSuggestion suggestionWithString:res]];
	}
	...
	// window.google.ac.h(["133",[["1337","42,300,000 results","0"],["1337 translator","1,120,000 results","1"],["1330 boylston","34,400 results","2"],["1337 google","2,350,000 results","3"],["1335 avenue of the americas","364,000 results","4"],["1337x.org","219,000 results","5"],["1337pwn","331,000 results","6"],["133t","207,000 results","7"],["133 east 64th street","19,600 results","8"],["1333 minna","50,300 results","9"]]])
// NEW VERSION
	...
	if ([result hasPrefix:magicStarter]) {
		...
		NSLog(@"wso: %@", wso);*/
		NSString *callToExtractSuggestions = [NSString stringWithFormat:@"extractGoogleSuggestSuggestions%@", [result substringFromIndex:[magicStarter length]]];
//		NSLog(@"query %@, extract: %@", string, callToExtractSuggestions);
		[self performSelectorOnMainThread:@selector(runSuggestionsParsing:) withObject:callToExtractSuggestions waitUntilDone:YES];
...
- (NSArray *)resultsForString:(NSString *)string {
	...
	NSURL *u = [NSURL URLWithString:[NSString stringWithFormat:@"http://ajax.googleapis.com/ajax/services/search/web?v=1.0&hl=en&q=%@", [string stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]]; //[NSURL fileURLWithPath:[[NSHomeDirectory() stringByAppendingPathComponent:@"Desktop"] stringByAppendingPathComponent:@"googlexhtmlquote.txt"]];
//	NSLog(@"U: %@", u);
//    NSURLConnection *conn = [[NSURLConnection alloc] ini]
	...
	NSData *dataFromGoogle = [NSURLConnection sendSynchronousRequest:request returningResponse:NULL error:NULL]; //[NSData dataWithContentsOfURL:u];
...
//		NSLog(@"title: %@", title);
		...
		[result setDescription:[[descr stringByTrimmingCharactersInSet:dashWhitespaceNewlineSet] gtm_stringByUnescapingFromHTML]];
