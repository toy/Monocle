...
- (BOOL)isFlat {
	...
	unsigned i = 0;
	for (i = 0; i < c; i++) {
		if ([col isNotEqualTo:[self colorStopAtIndex:i]])
			return NO;
	}
	return YES;
}
...
+ (void)drawBackgroundWash:(CTGradient *)gradient inFrame:(NSRect)rect sheen:(BOOL)hasSheen {
	...
	NSRect glassRect = NSMakeRect(rect.origin.x,rect.origin.y+NSHeight(rect)/2.0,NSWidth(rect),NSHeight(rect)/2.0);
	CTGradient *sheen = [CTGradient gradientWithBeginningColor:[[NSColor whiteColor] colorWithAlphaComponent:MonocleGradientSheenAlphaFrom]
...
+ (void)getPrimaryWash:(CTGradient **)gradient hasSheen:(BOOL *)sheen {
	...
	BOOL isGlass = ([[udc values] valueForKey:@"SearchBarHasSheen"] ? [(NSNumber *)[[udc values] valueForKey:@"SearchBarHasSheen"] boolValue] : NO);
...
+ (CTGradient *)primaryWash {
	CTGradient *g; [self getPrimaryWash:&g hasSheen:NULL];
	if (!g) g = nil;
	return g;
}
