...
- (BOOL)scanJSONString:(NSString **)string {
  ...
  if ([self scanJSONStringDelimiterString]) {
      ...
      if (currentChar != '\\') {
        ...
        switch (nextChar) {
          ...
          default:
            ...
            NSLog(@"nextchar: %C %d", nextChar, nextChar);
            break;
        }
        [self setScanLocation:([self scanLocation] + 2)];
      }
    ...
    /* this code is more appropriate if you have a separate method to unescape the found string
      for example, between inputting json and outputting it, it may make more sense to have a category on NSString to
    perform escaping and unescaping. Keeping this code and looking into this for a future update. NSUInteger
    searchLength = [[self string] length] - [self scanLocation]; NSUInteger quoteLocation = [[self string]
    rangeOfString:jsonStringDelimiterString options:0 range:NSMakeRange([self scanLocation], searchLength)].location;
    searchLength = [[self string] length] - quoteLocation;
    while (([[[self string] substringWithRange:NSMakeRange((quoteLocation - 1), 2)]
    isEqualToString:jsonStringEscapedDoubleQuoteString]) && (quoteLocation != NSNotFound) &&
         (![[[self string] substringWithRange:NSMakeRange((quoteLocation -2), 2)]
    isEqualToString:jsonStringEscapedSlashString])){ searchLength = [[self string] length] - (quoteLocation + 1);
      quoteLocation = [[self string] rangeOfString:jsonStringDelimiterString options:0 range:NSMakeRange((quoteLocation
    + 1), searchLength)].location;
    }

    *string = [[self string] substringWithRange:NSMakeRange([self scanLocation], (quoteLocation - [self
    scanLocation]))];
    // TODO: process escape sequences out of the string - replacing with their actual characters. a function that does
    just this belongs
    // in another class. So it may make more sense to change this whole implementation to just go character by character
    instead. [self setScanLocation:(quoteLocation + 1)];
    */
    result = YES;
  }
...
- (BOOL)scanJSONWhiteSpace {
  // NSLog(@"Scanning white space - here are the next ten chars ---%@---", [[self string]
  // substringWithRange:NSMakeRange([self scanLocation], 10)]);
  BOOL result = NO;
  ...
  // NSLog(@"Done Scanning white space - here are the next ten chars ---%@---", [[self string]
  // substringWithRange:NSMakeRange([self scanLocation], 10)]);
  return result;
}
