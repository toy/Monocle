...
#define SRAnimationOffsetRect(X, Y) \
  (SRAnimationAxisIsY ? NSOffsetRect(X, 0.0, -NSHeight(Y)) : NSOffsetRect(X, NSWidth(Y), 0.0))

@class SRRecorderControl, CTGradient, SRValidator;

enum SRRecorderStyle { SRGradientBorderStyle = 0, SRGreyStyle = 1 };
