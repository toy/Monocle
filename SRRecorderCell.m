  ...
  if (style == SRGradientBorderStyle) {
    ...
    if (!isRecording) {
      ...
      if (![self _isEmpty] && [self isEnabled]) {
        NSString *removeImageName = [NSString
          stringWithFormat:@"SRRemoveShortcut%@",
          (mouseInsideTrackingArea ? (mouseDown ? @"Pressed" : @"Rollover") : (mouseDown ? @"Rollover" : @""))];
    ...
    NSDictionary *attributes = [NSDictionary
      dictionaryWithObjectsAndKeys:mpstyle,
      NSParagraphStyleAttributeName,
      [NSFont systemFontOfSize:(recordingOrEmpty ? [NSFont labelFontSize] : [NSFont smallSystemFontSize])],
      NSFontAttributeName,
      (recordingOrEmpty ? [NSColor disabledControlTextColor] : [NSColor blackColor]),
    ...
    if (isRecording) {
      ...
      if (![self _validModifierFlags:recordingFlags]) {
        ...
        // Fall back on 'Type shortcut' if we don't have modifier flags to display; this will happen for the fn key
        // depressed
    ...
    textRect.size.width -=
      ((!isRecording && [self _isEmpty]) ? 6
                                         : (isRecording ? [self _snapbackRectForFrame:cellFrame].size.width
                                                        : [self _removeButtonRectForFrame:cellFrame].size.width) +
            6);
    textRect.origin.x += 6;
    ...
    double alphaRecording = 1.0;
    double alphaView = 1.0;
    if (isAnimatingNow && !isAnimatingTowardsRecording) {
      alphaRecording = 1.0 - xanim;
      alphaView = xanim;
    }
    if (isAnimatingNow && isAnimatingTowardsRecording) {
      alphaView = 1.0 - xanim;
      alphaRecording = xanim;
    }

    if (isAnimatingNow) {
      // NSLog(@"animation step: %f, effective: %f, alpha recording: %f, view: %f", transitionProgress, xanim,
      // alphaRecording, alphaView);
    }
    ...
    if (isVaguelyRecording) {
      if (isAnimatingNow) {
        //			[transitionMovement translateXBy:(isAnimatingTowardsRecording ? -(NSWidth(cellFrame)*(1.0-xanim)) :
        //+(NSWidth(cellFrame)*xanim)) yBy:0.0];
        currRecordingGradient = [currRecordingGradient gradientWithAlphaComponent:alphaRecording];
        if (SRAnimationAxisIsY) {
          //				[viewportMovement translateXBy:0.0 yBy:(isAnimatingTowardsRecording ? -(NSHeight(cellFrame)*(xanim))
          //: -(NSHeight(cellFrame)*(1.0-xanim)))];
          [viewportMovement translateXBy:0.0
                                     yBy:(isAnimatingTowardsRecording ? NSHeight(cellFrame) * (xanim)
                                                                      : NSHeight(cellFrame) * (1.0 - xanim))];
        } else {
          [viewportMovement translateXBy:(isAnimatingTowardsRecording ? -(NSWidth(cellFrame) * (xanim))
                                                                      : -(NSWidth(cellFrame) * (1.0 - xanim)))
                                     yBy:0.0];
        }
      } else {
        if (SRAnimationAxisIsY) {
    ...
    //	if (isVaguelyRecording)
    {
      roundedRect = [viewportMovement
        transformBezierPath:[NSBezierPath bezierPathWithSRCRoundRectInRect:SRAnimationOffsetRect(cellFrame, cellFrame)
      ...
      //		NSLog(@"snapbackrect: %@; offset: %@", NSStringFromRect([self _snapbackRectForFrame: cellFrame]),
      // NSStringFromRect(snapBackRect));
      ...
      [[((mouseDown && mouseInsideTrackingArea) ? ([CTGradient unifiedPressedGradient])
                                                : ([CTGradient unifiedNormalGradient]))
        gradientWithAlphaComponent:alphaRecording] fillRect:NSInsetRect(correctedSnapBackRect,
                                                              -([snapBackButton lineWidth] / 2.0),
                                                              -([snapBackButton lineWidth] / 2.0))
                                                      angle:90];
      /*
    // Highlight if inside or down
    ...
    // If key combination is set and valid, draw remove image
    if (![self _isEmpty] && [self isEnabled]) {
      NSString *removeImageName =
        [NSString stringWithFormat:@"SRRemoveShortcut%@",
                  (mouseInsideTrackingArea ? (mouseDown ? @"Pressed" : @"Rollover") : (mouseDown ? @"Rollover" : @""))];
      NSImage *removeImage = SRResIndImage(removeImageName);
      [removeImage dissolveToPoint:[viewportMovement transformPoint:([self _removeButtonRectForFrame:cellFrame].origin)]
                          fraction:alphaView];
...

    {
      ...
      NSDictionary *attributes = [NSDictionary
        ...
        [NSFont systemFontOfSize:(recordingOrEmpty ? [NSFont labelFontSize] : [NSFont smallSystemFontSize])],
        NSFontAttributeName,
        [(recordingOrEmpty ? [NSColor disabledControlTextColor]
                           : [NSColor blackColor]) colorWithAlphaComponent:alphaRecordingText],
        NSForegroundColorAttributeName,
        nil];
      // Recording, but no modifier keys down
      if (![self _validModifierFlags:recordingFlags]) {
...

    {
      ...
      NSDictionary *attributes = [NSDictionary
        dictionaryWithObjectsAndKeys:mpstyle,
        NSParagraphStyleAttributeName,
        [NSFont systemFontOfSize:([self _isEmpty] ? [NSFont labelFontSize] : [NSFont smallSystemFontSize])],
        NSFontAttributeName,
        [([self _isEmpty] ? [NSColor disabledControlTextColor]
                          : [NSColor blackColor]) colorWithAlphaComponent:alphaCombo],
        NSForegroundColorAttributeName,
        nil];
      // Not recording...
      if ([self _isEmpty]) {
...
- (void)resetTrackingRects {
  ...
  if (!isRecording) {
    ...
    if (snapbackTrackingRectTag != 0) [controlView removeTrackingRect:snapbackTrackingRectTag];
    snapbackTrackingRectTag = [controlView addTrackingRect:snapbackRect
                                                     owner:self
                                                  userData:nil
                                              assumeInside:mouseInside];
...
- (BOOL)trackMouse:(NSEvent *)theEvent
  ...
  NSRect trackingRect =
    (isRecording ? [self _snapbackRectForFrame:cellFrame] : [self _removeButtonRectForFrame:cellFrame]);
...
- (BOOL)performKeyEquivalent:(NSEvent *)theEvent {
  ...
  BOOL validModifiers =
    [self _validModifierFlags:(snapback) ? [theEvent modifierFlags]
                                         : flags];  // Snapback key shouldn't interfer with required flags!
  ...
  if (isRecording && (validModifiers || snapback)) {
      ...
      if (goAhead) {
        ...
        if ([character length]) {
          ...
          if ([validator isKeyCode:[theEvent keyCode]
            ...
            hasKeyChars = YES;
            keyChars = [eventCharacters retain];
            ...
            //						NSLog(@"calculated keychars: %@, ignoring: %@", SRStringForKeyCode(keyCombo.code),
    ...
    if (([[[self controlView] window] firstResponder] == [self controlView]) && ([self isEnabled]) &&
...
- (NSString *)keyComboString {
  ...
  return [NSString
    stringWithFormat:@"%@%@", SRStringForCocoaModifierFlags(keyCombo.flags), SRStringForKeyCode(keyCombo.code)];
}
...
- (void)_privateInit {
  ...
  cancelCharacterSet = [[NSSet alloc] initWithObjects:[NSNumber numberWithInt:ShortcutRecorderEscapeKey],
                                      [NSNumber numberWithInt:ShortcutRecorderBackspaceKey],
                                      [NSNumber numberWithInt:ShortcutRecorderDeleteKey],
                                      nil];

  NSNotificationCenter *notificationCenter = [NSNotificationCenter defaultCenter];
  [notificationCenter addObserver:self
                         selector:@selector(_createGradient)
                             name:NSSystemColorsDidChangeNotification
                           object:nil];  // recreate gradient if needed
...
- (void)_createGradient {
  NSColor *gradientStartColor =
    [[[NSColor alternateSelectedControlColor] shadowWithLevel:0.2] colorWithAlphaComponent:0.9];
  NSColor *gradientEndColor =
    [[[NSColor alternateSelectedControlColor] highlightWithLevel:0.2] colorWithAlphaComponent:0.9];
...
- (void)_startRecordingTransition {
  if ([self _effectiveIsAnimating]) {
    ...
    [self performSelector:@selector(_transitionTick)
               withObject:nil
               afterDelay:(SRTransitionDuration / SRTransitionFrames)];
...
- (void)_saveKeyCombo {
  ...
  if (defaultsKey != nil && [defaultsKey length]) {
    ...
    NSDictionary *defaultsValue = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithShort:keyCombo.code],
                                                @"keyCode",
                                                [NSNumber numberWithUnsignedInt:keyCombo.flags],
                                                @"modifierFlags",  // cocoa
                                                [NSNumber numberWithUnsignedInt:SRCocoaToCarbonFlags(keyCombo.flags)],
                                                @"modifiers",  // carbon, for compatibility with PTKeyCombo
                                                nil];
...
- (void)_loadKeyCombo {
  ...
  if (defaultsKey != nil && [defaultsKey length]) {
    ...
    unsigned int flags;
    if ((nil == [savedCombo valueForKey:@"modifierFlags"]) &&
      (nil != [savedCombo valueForKey:@"modifiers"])) {  // carbon, for compatibility with PTKeyCombo
...
- (NSRect)_removeButtonRectForFrame:(NSRect)cellFrame {
  ...
  NSSize originalSize = [removeImage size];
  double scaleFactor = [[NSScreen mainScreen] userSpaceScaleFactor];
  NSSize demultipliedSize = NSMakeSize(originalSize.width / scaleFactor, originalSize.height / scaleFactor);

  removeButtonRect.origin =
    NSMakePoint(NSMaxX(cellFrame) - demultipliedSize.width - 4, (NSMidY(cellFrame) - (demultipliedSize.height / 2)));
  removeButtonRect.size = originalSize;
...
- (NSRect)_snapbackRectForFrame:(NSRect)cellFrame {
  ...
  NSSize originalSize = [snapbackImage size];
  double scaleFactor = [[NSScreen mainScreen] userSpaceScaleFactor];
  NSSize demultipliedSize = NSMakeSize(originalSize.width / scaleFactor, originalSize.height / scaleFactor);

  snapbackRect.origin =
    NSMakePoint(NSMaxX(cellFrame) - demultipliedSize.width - 2, (NSMaxY(cellFrame) - demultipliedSize.height) / 2 + 1);
  snapbackRect.size = originalSize;
...
- (BOOL)_validModifierFlags:(unsigned int)flags {
  return (allowsKeyOnly ? YES
                        : (((flags & NSCommandKeyMask) || (flags & NSAlternateKeyMask) || (flags & NSControlKeyMask) ||
                             (flags & NSShiftKeyMask) || (flags & NSFunctionKeyMask))
                              ? YES
                              : NO));
}
